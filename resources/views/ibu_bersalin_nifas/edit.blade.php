...
@section('content')
    <!-- Page Heading -->
    <h1 class="h3 mb-4 text-gray-800">{{ $title ?? __('Update Ibu Bersalin Nifas') }}</h1>
    ...
    <div class="card">
        <div class="card-body" style="text-align: center;">
            ...
            <form action="{{ route('ibu_bersalin_nifas.update', ['id' => $ibuhamilNifas->id]) }}" method="post"
                id="modal-save-form">
                @csrf
                @method('PUT')
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('nama') is-invalid @enderror"
                                    ...
                                    value="{{ old('nama') ?? $ibuhamilNifas->nama }}" required>
                                @error('nama')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control @error('nik') is-invalid @enderror" name="nik"
                                    ...
                                    value="{{ old('nik') ?? $ibuhamilNifas->nik }}" required>
                                @error('nik')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                ...
                <div class="card" style="margin-top: 2%">
                    ...
                    <a href="{{ route('ibu_bersalin_nifas.index') }}" class="btn btn-default">Kembali Ke Halaman
                        List</a>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // Function to toggle form fields based on status
            function toggleFormFields(status) {
                ...
                for (var key in fields) {
                    ...
                    fields[key].value = isActive ? fields[key].value : ''; // Clear value if not active
                }
            }

            // Set up the event listener for the status select field
            statusSelect.addEventListener('change', function() {
                toggleFormFields(statusSelect.value);
            });

            // Initial check based on the current value of the status select field
            toggleFormFields(statusSelect.value);
            ...
            // Remove local storage data on form submit
            ...
            // Function to toggle additional fields for "lainnya" options
            function initializeSelect(selectId, fieldId) {
                var selectElement = document.getElementById(selectId);
                var fieldElement = document.getElementById(fieldId);
                var oldValue = fieldElement.getAttribute('data-old');

                function toggleFields() {
                    var isActive = selectElement.value === 'lainnya';
                    fieldElement.disabled = !isActive;
                    fieldElement.value = isActive ? (oldValue || '') : null; // Set to null when not active
                }
            ...
            // Initialize "lainnya" fields for penolong_persalinan and cara_persalinan
            initializeSelect('penolong_persalinan', 'lainya_penolong_persalinan');
            initializeSelect('cara_persalinan', 'lainya_cara_persalinan');
        });
    </script>
@endsection
