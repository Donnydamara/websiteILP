...
class RekapitulasiController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Jika tidak ada tanggal mulai, tentukan berdasarkan periode
        if (!$startDate) {
        ...
        // Ambil filter user dan posyandu dari request
        $userId = $request->get('user_id');
        $posyandu = $request->get('posyandu');

        // Ambil data mingguan berdasarkan tanggal, user, dan posyandu
        $dataPerWeek = $this->getDataPerWeek($startDate, $endDate, $userId, $posyandu);

        // Ambil daftar user dan posyandu dari model JadwalPengumpulan
        $users = \App\Models\JadwalPengumpulan::select('user_id', 'name')->distinct()->get();
        ...
        if ($request->get('export') === 'excel') {
            ...
            // Kirim data, tanggal mulai, dan tanggal akhir ke RekapitulasiExport
        ...
        return view('rekapitulasi_kunjungan_rumah.list', compact('dataPerWeek', 'period', 'users', 'posyandus'));
    }
